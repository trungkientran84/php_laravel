<!--
* This Vue.js component is used to generate the slide bar on user's pages
* The component receives the menu json dynamically through php code.
*
-->
        ...
        <li v-for="(item, i) in items" :class="classes(item)">
            ...
            <div v-if="item.children.length > 0" :id="item.id+'-dropdown-element'" :class="'panel-collapse collapse' + (item.active ? ' in' : ' ')">
                <div class="panel-body">
                    <user-menu :items="item.children"></user-menu>
                </div>
            </div>
        </li>
    </ul>
